...
Use these instructions to generate fixity information for very large accessions. In most cases, fixity is generated by bagging the accession instead.

These instructions are for using the utility md5sum from the terminal, which is native to Mac and Linux operating systems. The end result is a single text file with the MD5 and filename for every file in the accession.


### 1. Change directory to the accession folder

`$ cd 'path/to/accession/folder'`

### 2. Calculate the MD5 and save as text file

 Use the [md5sum utility](https://linux.die.net/man/1/md5sum). The output is saved to a single text file in the accession folder and named accession-number_md5-manifest.txt. Replace "accession-number" in the command below with the actual accession number.

`$ find . -type f -exec md5sum {} \; > accession-number_manifest-md5.txt`

Output for running above command on accession 2017_04_er:

![md5sum_output](./images/md5.jpg)
